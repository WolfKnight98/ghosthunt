if ( CLIENT ) then
	...
	SWEP.HoldType			= "slam"	
	SWEP.PrintName			= "Green Glowstick"
...
SWEP.Primary.Sound          = "glowstick/glowstick_snap.wav"
...
SWEP.Secondary.Sound        = "glowstick/glowstick_snap.wav"
...
function SWEP:Deploy()
	...
	if ( SERVER ) then
		local glow = ents.Create( "ent_glowstick_glow_green" )
		glow:SetOwner( self.Owner ) 
		glow:SetParent( self.Owner )
		glow:SetPos( self.Owner:GetPos() )
		
		if ( self.Owner:IsBot() ) then self:SetColor( Color( 0, 255, 0, 255 ) ) else glow:SetColor( 0, 255, 0, 255 ) end
		glow:Spawn()
	end
	return true
end
...
function SWEP:PrimaryAttack()
	...
	timer.Simple(0.5, function()
		...
		if ( SERVER ) then
			...
			if ( self.Owner:IsBot() ) then ent:SetColor( Color( 0, 255, 0, 255 ) ) else ent:SetColor( Color( 0, 255, 0, 255 ) ) end
			ent:Spawn()
			ent:Activate()
...
function SWEP:Holster()
	...
	for k, v in pairs( worldmodel ) do 
		if ( v:GetClass() == "ent_glowstick_glow_green" and v:GetOwner() == self.Owner ) then
			v:Remove()
		end
	end
